...
- 用户触控或拖动滚动条时，虚拟 DOM 在同一帧收到合并后的滚动提示，不再额外等待一帧才更新挂载窗口；内部滚动写回仍不会冒充用户输入。
- 活跃滚动期间只保留楼层根高与内容尺寸的最后样本，停稳后再把请求水合、图片和回复线变化合入同一虚拟帧，减少滚动中的卡顿、空白和位置跳动。
- 前方正文按批次共用一次投影同步与物理视口补偿；回复关系线只在绘制期间再次失效时补帧，避免重复提交同一批 DOM 变化。

### 移动端交互

- Reader 内部历史回退改为捕获并消费 `popstate`，逐层执行既有 `Esc` 关闭链时不再把事件传播给宿主，避免 Reader 关闭过程触发宿主刷新或路由切换。
- 触屏标题操作展开后忽略触控结束产生的 `pointerleave`，避免菜单提前收起与点击穿透；纯触屏手机隐藏冗余沉浸入口，长标题允许换行并继续缩小。
- 主题编辑表单按可用宽度自动从双列收为单列，窄屏输入项不再撑出阅读器边界。

### 触摸缩放与交互减负

- 移动端图片灯箱支持以双指中心为锚点连续缩放与平移，手势结束后会正确释放捕获状态，并继续复用同一图片变换 owner。
- 回复树的滚动冻结投影改为写时复制，普通首滑不再同步复制整棵关系树；连续 wheel 或 touch 手势只在会话开始时广播一次高成本滚动意图。
- 楼层操作收纳、浮窗拖动、设置色盘和 Topic 操作列只在真实交互期间绑定移动监听；图片悬浮工具监听改为被动模式，减少移动端首帧主线程工作。
- 窄屏 Composer 显式继承阅读器编辑字号，系统字体渲染和 Reader 定位模式下保持一致。

### 发布状态

- `1.6.1` 已发布到 Greasy Fork：主 Loader 固定版本为 `1909752`，加载 Core `1909749`、Platform `1909731` 与 Features `1909750`。
- 固定 Loader 原始文件为 4,202 字节，SHA-256 `c3322f51e8facfd201e31775675a7d66651a2eb9d2fc7d014c8064c719447aa6`；移除平台加入的 `@downloadURL none` 后为 4,181 字节，SHA-256 `d6a6630a583d940ab8f5b302b99108edc746ed5ec8bd82c0151faacf2a609395`，与仓库 Loader 逐字节一致。
- Core 为 1,730,972 字节，SHA-256 `5b71ab3be91ae5b966c3b0c95140d3bd012759cb03b06740fdd00a4780f0c544`；Platform 为 1,482,147 字节，SHA-256 `f3fc41247dd79a34e9420af92d8a23fc644c4b20309269d3f0dae947a67aa033`；Features 为 1,993,857 字节，SHA-256 `2548c22f4323304e34529ecc4a242cbe46c0368e1c0b602b1e4563d176c58509`。
- CSS 固定到 Git `36ca6edefacac79bdaa3243932000671eba1d14e`：688,457 字节，SHA-256 `93e59f079e926511f6c8539e2496646791955cef01d8f70e144a0dbecc27258b`；源码与分包 runtime 各通过 234 文件契约，305 个模块通过 parity 门禁。
...
- 字体设置新增 11 种精选 Google Fonts，仅在实际选中时联网加载；主题摘要的字体选择、预览、下载和上传分享图复用同一目录与加载状态。
- 支持导入 WOFF2、WOFF、TTF 和 OTF 字体文件：单个不超过 32 MB、最多 64 个、合计不超过 128 MB；文件独立保存在当前浏览器的 IndexedDB，不进入设置导出、普通缓存清理或 WebDAV。
- 本机字体、精选字体和导入字体继续按来源分组并显示实际字形预览；长列表扩大宽度并统一获得可滚动高度，不再因可用空间较小而只剩搜索框。
...
- 新增统一信息流协调器，把设置、历史、岁月史书、不想看、阅读队列、用户观察、通知、收藏、下载、站点、翻译和 WebDAV 等存活期数据接入按领域刷新的跨标签页链路。
- 浏览器存储、中央缓存广播与 userscript value-change 统一合并同一 tick 和在飞期间的重复事件；只重读对应持久 owner，不触发整页刷新、额外网络请求或无关领域更新。
- userscript 环境补齐现代与兼容形态的 GM value-change 监听，并在页面生命周期结束时释放 storage、cache 和 GM 订阅，关闭页面不再响应晚到事件。
- 请求许可、缓存单飞租约、一次性重载事务和资源缓存继续保留各自协议 owner，不被业务信息广播误投影。

### 阅读与交互修复

- 阅读队列、通知、收藏、Topic 下载、用户观察、Connect 信任记录与 Credit 账户投影可接收其他标签的持久变更，并保持筛选、分页和账号范围不串域。
- 宿主列表中的自动不想看命中只动态隐藏主题，不再写成手动免打扰记录；手动记录仍可筛选、追溯和批量恢复。
- 离线 Topic 补齐 inline onebox、来源图标、缩略图和链接点击数投影；主题预热、离线归档与跨标签缓存失效继续遵守当前请求和账户边界。
- 灯箱说明区域改为内容驱动的高度与滚动上限，主题摘要和共享多标签浮窗复用稳定几何；原站编辑器、宿主主题卡片和队列操作边界同步修复。

### 发布状态

- `1.5.2` 已发布到 Greasy Fork：主 Loader 固定版本为 `1904498`，加载 Core `1904487`、Platform `1904489` 与 Features `1904488`。
- 固定 Loader 原始文件为 4,141 字节，SHA-256 `8e2f3aa1b2dabeaf2ca276ddc639f864cfa78b8a50179bd38c824dadb2f15d2e`；移除平台加入的 `@downloadURL none` 后为 4,120 字节，SHA-256 `5265fd8e3496c432938053bdd3e19b01e64b316aff401ebfa80d50a68af4c216`，与仓库 Loader 逐字节一致。
- Core 为 1,572,987 字节，SHA-256 `d06e345a438a2255c27251f61c9d29810034841ca9b52a565463ac35d461f938`；Platform 为 1,244,026 字节，SHA-256 `00e240f9860bae3179c863809a76dc847260bab683189126d2f499d2ab793ea1`；Features 为 1,965,693 字节，SHA-256 `818da987ac607ab5253394779b56128ce58a1f22cea8c35b8978d87fc6356204`。
- CSS 固定到 Git `7c25413b2eb2b2e9a30d33c5a346f858751eb1cc`：610,968 字节，SHA-256 `4c6ae29067770d2553befee1c7ac7ddb61d347abdfb6fa8bf23b6c50f52f10b1`；源码与分包 runtime 各通过 223 文件契约，287 个模块通过 parity 门禁。
...
- 岁月史书保留服务器已 404、但本机仍有正文快照的 Topic；不想看支持类别、标签、楼主、楼层用户和字符规则，自动命中只动态隐藏，手动免打扰记录可筛选和批量恢复。

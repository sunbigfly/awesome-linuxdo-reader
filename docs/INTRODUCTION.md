...
本文集中维护 GitHub 与 Greasy Fork 可复用的对外介绍文案。当前版本：`1.3.0`。
...
阅读器围绕长内容场景提供楼层关系、楼中楼、跳转提示、浏览历史、真实已读进度、原图灯箱、媒体播放与公式渲染。针对长帖和多标签页场景，它还提供 DOM 窗口化、缓存分层、共享请求调度、429 退避、性能配置和本地资源监控，尽量在阅读连贯性与站点负载之间取得平衡。WebDAV 可按类别三方合并有价值的本地记录；翻译服务配置与译文缓存只有用户主动选择后才会同步。1.3.0 新增后台 Topic 下载、可搜索和跳转的独立离线 HTML、下载历史及默认关闭的 WebDAV 离线同步；设置导出、导入和恢复默认也收敛为可回滚的组合事务，翻译 API Key、WebDAV 用户名与密码始终排除。

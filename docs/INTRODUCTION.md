...
为 LINUX DO 深度定制并保持完整功能覆盖，同时全面兼容标准 Discourse 社区、支持非中文正文翻译的沉浸式增强阅读器。

## 简短介绍

Awesome LinuxDo Reader 是一款运行在浏览器 userscript 管理器中的 Discourse 增强阅读工具。它对 LINUX DO 的消息、历史、收藏、互动、长帖和楼层关系做了完整适配，同时通过运行态识别、站点适配层和插件能力检测兼容标准 Discourse 社区；在保留原站账号与内容的同时，提供非中文正文翻译、原图查看、历史导航和个性化布局。
...
LINUX DO 全面适配，标准 Discourse 站点通用；可在列表页直接阅读完整帖子、保留楼层关系与原站互动，并切换非中文正文的原文、双语和全译文。

...
::: tip 当前发布状态
Lite `1.1.0` 源码与本手册正在准备发布；Greasy Fork 脚本 588185 当前公开稳定版仍为 `1.0.1`。1.0.1 主脚本固定版本为 `1895932`，加载 Core `1895921` 和 Features `1895924`。完成 1.1.0 的三文件同步与浏览器门禁后，本页再更新新的固定坐标。
:::
...
5. 其他站点可先在任一已支持论坛打开阅读器，通过“设置 → 适用站点 → 其他适用站点”输入域名或网址；脚本验证其为 Discourse 后才会保存。
6. 在主题列表顶部或主题链接附近确认阅读器入口已经出现。

::: tip 站点与翻译能力
LINUX DO 保持完整功能覆盖。中文、英文及其他语言的标准 HTTPS Discourse 论坛通过验证后即可使用核心阅读能力，站点未安装的 Boost、Reactions、Post Voting 等插件入口会自动隐藏。站点适配不限内容语言；除内置中文站点 LINUX DO 外，其他内置社区和语言未知的自定义 Discourse 站点会显示译为简体中文的正文翻译按钮。
:::
...
- 在“设置 → 关于”或脚本管理器中确认版本；本手册当前对应 `1.1.0`。

...
新增类别、Label、OP 或楼层用户时会自动查询宿主，只有选择合法候选后才能加入；同名用户需根据唯一用户名和 ID 二次选择。字符匹配可指定 `title`、`category`、`label`、`user` 或 `topic`，支持普通包含与正则表达式。保存后会立即重扫当前 Topic 列表和已挂载楼层，不需要刷新页面。主题类别、Label、OP 与字符规则在动态隐藏列表项的同时会写入自动过滤历史；楼层用户命中也会把当前 Topic 与用户原因记入历史，但只隐藏该用户的楼层本体并保留回复树。自动历史不拥有永久隐藏权：关闭总开关或删除规则后已挂载内容立即恢复，历史仍可追溯；自动历史条目直接进入规则管理，不参与批量恢复。只有手动免打扰会持久收纳 Topic，直到用户点击恢复显示。

数据管理中的全局“恢复全部默认”会保留手动收纳记录、自动过滤历史和上述全部自动过滤设置。本面板自己的“恢复默认”仅重置自动过滤草稿，保存后生效；它不会删除“不想再看”中的既有历史。

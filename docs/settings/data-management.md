...
<p class="image-caption">设置契约升级时会显示一次性提示；选择重置只恢复普通阅读器设置和队列图标，保留“不想再看”内容、队列条目、历史、缓存、账号数据和原站内容。</p>
...
| 收藏、回应与其他数据 | 收藏列表、回应、点赞、译文、公共 AI 模型元数据及其他已登记通用响应 | 原站真实收藏与回应、AI 服务配置、供应商模型目录、主题总结历史 |
...
缓存不覆盖阅读器的每一种业务状态，也不应该这样做。偏好、阅读队列、离线 Topic、导入字体、AI 服务配置和主题总结历史是用户数据；发帖、回应、上传、权限与已读提交是服务端权威动作；普通图片字节由浏览器 HTTP cache 管理。它们都有独立 owner 和清理入口，不会为了统一名称被塞进六类可重取缓存。

统一响应缓存使用 IndexedDB `awesome linuxdo reader`。从旧版升级时，阅读器会把 `linuxdo-enhanced-reader:responses:v1` 中的中央响应记录按更新时间合并到新库；新库事务成功后才尝试删除旧库。若其他标签仍占用旧库或删除失败，旧库会保留并在下次启动继续迁移，但当前版本只从新库读取，不会形成双库同时命中。独立导入字体仓储不参与这次迁移。
...
清理事务会先暂停通知、收藏等可能继续写缓存的后台任务，让正在返回的旧异步结果失效，再清应用内存、IndexedDB 和跨标签副本；持久层完成后才重建当前主题并恢复后台任务。跨标签广播失败、数据库删除失败或应用热缓存清理失败都会保留对应勾选并显示部分完成，不再把残留状态误报为成功。

清理主题与楼层缓存会增加下一次打开主题的请求，但不会删除已经生成并保存的主题总结历史。清理用户资料不会删除账号或观察名单，但会清空观察公开历史、采集断点、待续传任务和当前会话内存投影，避免旧观察结果在持久层清空后重新写回。清理消息缓存不会把通知标记为已读。

清理“收藏、回应与其他数据”会一并删除可匿名重新获取的公共 AI 模型元数据：GM 持久值、当前运行时内存目录和设置页投影会同步清空，正在返回的旧请求会被中止，其他标签也会收到清空状态。供应商 `/models` 返回并随 AI 服务配置保存的目录不会删除，避免把业务模型选择误当临时缓存。

...
const stylesheetPath = path.join(projectRoot, 'work/mian-lite.css');
...
const missingLayoutContracts = requiredLayoutContracts
	...
	.map(([name]) => name);
if (forbidden.length) {
	throw new Error(`mian-lite.css 命中禁止项：${forbidden.join(', ')}`);
}
if (missingLayoutContracts.length) {
	throw new Error(
		`mian-lite.css 缺少像素布局契约：${missingLayoutContracts.join(', ')}`,
	);
}
if (unownedCssClasses.length) {
	throw new Error(
		`mian-lite.css 存在无运行时所有者的 class：${unownedCssClasses.join(', ')}`,
	);
}
if (unscopedSelectors.length) {
	throw new Error(
		`mian-lite.css 存在未锚定到 Lite surface 的 selector：` +
		unscopedSelectors.slice(0, 20).join(' | '),
	);
}
if (uncoveredRuntimeClassGroups.length) {
	throw new Error(
		'mian-lite.css 存在未覆盖的运行时 DOM class 组：' +
...
if (unexpectedUnstyledRuntimeClasses.length) {
	throw new Error(
		'mian-lite.css 存在未分类的无样式运行时 class：' +

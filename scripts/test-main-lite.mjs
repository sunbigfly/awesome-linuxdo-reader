...
if (result.warnings.length) {
	throw new Error(`main-lite 测试构建返回 ${result.warnings.length} 条警告`);
}
const outputFile = result.outputFiles?.[0];
if (!outputFile) throw new Error('main-lite 测试构建无输出');
...
try {
	...
	process.stderr.write(`main-lite contract tests: failed: ${name}: ${message}\n`);
	process.exitCode = 1;
	process.exit();
}
process.stdout.write(`main-lite contract tests: passed (${testFiles.length} files)\n`);

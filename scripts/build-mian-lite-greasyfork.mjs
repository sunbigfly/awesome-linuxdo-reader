...
const releaseOutputPath = path.join(projectRoot, 'work/mian-lite.js')
...
const entryModuleId = 'src/userscript/mian-lite-entry.js'
...
\t\t\tif (!request.startsWith('.')) {
\t\t\t\tthrow new Error(\`[mian-lite] unsupported external module: \${request}\`);
\t\t\t}
\t\t\tconst parts = parentId.split('/');
\t\t\tparts.pop();
\t\t\tfor (const part of request.split('/')) {
...
\t\t\t\tif (part === '..') {
\t\t\t\t\tif (!parts.length) {
\t\t\t\t\t\tthrow new Error(\`[mian-lite] module escapes root: \${parentId} -> \${request}\`);
\t\t\t\t\t}
\t\t\t\t\tparts.pop();
\t\t\t\t} else {
\t\t\t\t\tparts.push(part);
...
\t\t\tif (!factory) throw new Error(\`[mian-lite] missing module: \${id}\`);
...
\t\t\t\t\t\tthrow new Error(\`[mian-lite] conflicting module: \${id}\`);
\t\t\t\t\t}
\t\t\t\t\treturn;
\t\t\t\t}
...
\t\t\t\t\t\tthrow new Error(\`[mian-lite] missing library: \${name}\`);
\t\t\t\t\t}
\t\t\t\t}
...
function validateStylesheetUrl(value, stylesheetHash) {
  ...
  if (!match) {
    throw new Error('readerStylesUrl 必须是带 commit 与 sha256 的不可变 mian-lite.css jsDelivr URL')
  }
  if (match[2].toLowerCase() !== stylesheetHash) {
    throw new Error('readerStylesUrl 的 SHA-256 与当前 work/mian-lite.css 不一致')
  }
  return String(value)
}
...
if (config) {
  ...
  await emit(releaseOutputPath, releaseLoader, check)
  release = Object.freeze({
    file: slash(path.relative(projectRoot, releaseOutputPath)),

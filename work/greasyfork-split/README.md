...
- `main.user.js`：从当前 `work/main.js` 按字节复制，0.1.15 基线为 1,790,791 bytes，暂未接入 Core 或新资源。
- `main.css`：从当前 `work/main.css` 按字节复制，0.1.15 基线为 414,318 bytes。
- `reader-core.js`：由当前单一 IIFE 生成的 Core Library，0.1.15 为 1,788,802 bytes；加载后只注册命名空间，不会自动启动。

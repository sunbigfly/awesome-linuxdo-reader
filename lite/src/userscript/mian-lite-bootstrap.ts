...
const DEBUG_HANDLE_KEY = '__LDP_MIAN_LITE__';
...
type MianLitePost = ReaderPostPresentationPost
	& DiscourseComposerPostInput
	& ReaderLightboxCommentPostInput
	& CanonicalActionPost
	& Readonly<Record<string, unknown>>;

interface MianLiteTopic extends DiscourseComposerTopicInput<MianLitePost> {
	readonly [key: string]: unknown;
}

export interface MianLiteUserscriptHandle {
	...
	readonly runtime: ReaderBrowserRuntime<MianLiteTopic, MianLitePost> | null;
	destroy(): void;
}

interface MutableMianLiteState {
	runtime: ReaderBrowserRuntime<MianLiteTopic, MianLitePost> | null;
...
function pageRecord(value: unknown): Record<string, unknown> {
	if (value === null || (
		...
		throw new Error('mian-lite page window 不可用');
	}
	...
	return Object.freeze({
		...
		setup(
			...
			const replyTreePreferences =
				new ReaderReplyTreePreferencesPreview(
					...
					(error) => {
						console.error('[mian-lite:reply-tree-preview]', error);
					},
				);
		...
		shell: {
			...
			createView: () => {
				...
				if (!portal) throw new Error('mian-lite Shadow Portal 未就绪');
			...
			createWorkspaceOptions: (_shell, context) => {
				if (!template) throw new Error('mian-lite Shell template 未创建');
		...
		runtime: {
			...
			topicFactory: {
				createDomOptions: (bundle, context, _root, services) => {
					const presentation =
						createReaderPostPresentation<MianLitePost>({
							document,
					...
					const readState = createReaderPostReadStateFeature<MianLitePost>({
					...
					const readViewport =
						new ReaderPostReadViewportFeature<MianLitePost>({
		...
		targets: {
			selectOpenTopicsAtFirstPost: (preferences) =>
				preferences.openTopicsAtFirstPost,
			onError: (error) => {
				console.error('[mian-lite:target]', error);
			},
		},
		onReady(runtime, context, _settings, _settingsView, _layout, appearance, font) {
			...
			const embeddedReload = routeKind === 'list'
				? new ReaderEmbeddedReloadCoordinator({
					...
					onError: (error) => {
						console.error('[mian-lite:embedded-reload]', error);
					},
				})
				: null;
			void (async () => {
				...
				console.error('[mian-lite:embedded-default-topic]', error);
			});
			const composerWindow = new ReaderNativeComposerWindowController({
				...
				parentScope: runtime.scope,
				onError: (error) => {
					console.error('[mian-lite:native-composer-window]', error);
				},
			});
...
export function startMianLiteUserscript(
	...
	if (document.location.hostname === 'credit.linux.do') {
		scheduleReaderCreditAccountBridge(
			...
			(cause) => console.warn('[mian-lite] LDC 账户桥同步失败', cause),
		);
		return null;
	}
	...
	if (existing) return existing as MianLiteUserscriptHandle;
	...
	const state: MutableMianLiteState = {
		runtime: null,
		portal: null,
		diagnostics: [],
	};
	...
	const application = createReaderUserscriptApplication({
		...
		stages: [
			createPreferencesStorageSyncStage({
				...
				onError: (cause) => {
					console.error('[mian-lite:preferences-sync]', cause);
				},
			}),
	...
	application.diagnostics.subscribe((diagnostic) => {
		...
		console.error(
			`[mian-lite:${diagnostic.stage}]`,
			diagnostic.cause,
		);
	});
	const started = application.start();
	const handle: MianLiteUserscriptHandle = Object.freeze({
		application,
		started,
		get diagnostics() {
